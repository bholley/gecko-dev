...
nsRefPtr<MediaSourceReader::AudioDataPromise>
MediaSourceReader::RequestAudioData()
{
  AutoReturnMediaPromise<AudioDataPromise> promise(&mAudioPromise,
                                                   "MediaSourceReader::RequestAudioData");
  ...
  mAudioReader->RequestAudioData()->Then(GetTaskQueue(), this,
...
{
  AutoServiceMediaPromise<AudioDataPromise> promise(&mAudioPromise, "MediaSourceReader::OnAudioDecoded");
...
MediaSourceReader::OnAudioNotDecoded(NotDecodedReason aReason)
{
  ...
  // give us more data.
  promise.Reject(WAITING_FOR_DATA);
}
...
MediaSourceReader::RequestVideoData(bool aSkipToNextKeyframe, int64_t aTimeThreshold)
{
  AutoReturnMediaPromise<VideoDataPromise> promise(&mVideoPromise,
                                                   "MediaSourceReader::RequestVideoData");
...
{
  AutoServiceMediaPromise<VideoDataPromise> promise(&mVideoPromise,
  ...
  if (mDropVideoBeforeThreshold) {
    if (aSample->mTime < mTimeThreshold) {
      MSE_DEBUG("MediaSourceReader(%p)::OnVideoDecoded mTime=%lld < mTimeThreshold=%lld",
                this, aSample->mTime, mTimeThreshold);
      mVideoReader->RequestVideoData(false, 0)->Then(GetTaskQueue(), this,
                                                     &MediaSourceReader::OnVideoDecoded,
