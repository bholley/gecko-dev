...
ScriptSettingsStackEntry ScriptSettingsStackEntry::BarrierSingleton;
...
public:
  ...
  void Push(ScriptSettingsStackEntry* aSettings) {
    ...
    MOZ_ASSERT_IF(mStack.Length() == 0 || mStack.LastElement()->IsBarrier(),
                  aSettings->mIsCandidateEntryPoint);
    mStack.AppendElement(aSettings);
  }

  void PushBarrier() {
    mStack.AppendElement(&ScriptSettingsStackEntry::BarrierSingleton);
  }
...
GetWebIDLCallerPrincipal()
{
  ...
  // If we have an entry point that is not the barrier singleton, we know it
  // must be an AutoEntryScript.
  if (!entry || entry->IsBarrier()) {
    return nullptr;
  }
