...
GetOrCreateClassObjectMap(JSContext *cx, JS::Handle<JSObject*> scope, const char *mapName)
{
  ...
  if (desc.object() && desc.value().isObject() &&
      JS_IsWeakMapObject(&desc.value().toObject())) {
    return &desc.value().toObject();
  }

  // It's not there. Create and define it.
  JS::Rooted<JSObject*> map(cx, JS_NewWeakMapObject(cx));
...
GetOrCreateMapEntryForPrototype(JSContext *cx, JS::Handle<JSObject*> proto)
{
  ...
  // collisions, since we can simultaneously  have a bound in-content node with
  // grand-proto HTMLDivElement and a bound anonymous node whose grand-proto is
  // the XBL scope's cross-compartment wrapper to HTMLDivElement. Since we have
  // to wrap the WeakMap keys into its scope, this distinction would be lost if
  ...
  JS::RootedObject scope(cx, xpc::GetXBLScopeOrGlobal(cx, proto));
  JS::RootedObject wrappedProto(cx, proto);
  ...
  JS::RootedObject map(cx, GetOrCreateClassObjectMap(cx, scope, name));
  if (!map) {
    return nullptr;
  }
  ...
  if (!JS_GetWeakMapEntry(cx, map, wrappedProto, &val)) {
    return nullptr;
  }
  if (val.isObject()) {
    return &val.toObject();
  ...
  if (!JS_SetWeakMapEntry(cx, map, wrappedProto, entryVal)) {
    return nullptr;
  }
  return entry;
}
                            ...
                            bool* aNew)
{
  ...
  JS::RootedObject xblScope(cx, xpc::GetXBLScopeOrGlobal(cx, global));
