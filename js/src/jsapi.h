 ...
 *
 * If a the embedding currently has a scripted caller override set for the
 * topmost activation record, this will also return false.
 */
extern JS_PUBLIC_API(bool)
JS_DescribeScriptedCaller(JSContext *cx, JS::MutableHandleScript script, unsigned *lineno);

/*
 * Informs the JS engine of the presence of a scripted caller override.
 *
...
class JSAutoScriptedCallerOverride
{
  public:
    JSAutoScriptedCallerOverride(JSContext *cx
                                 MOZ_GUARD_OBJECT_NOTIFIER_PARAM)
      : mContext(cx)
    {
        MOZ_GUARD_OBJECT_NOTIFIER_INIT;
        JS_AddScriptedCallerOverride(mContext);
    }
    ~JSAutoScriptedCallerOverride() {
        JS_RemoveScriptedCallerOverride(mContext);
    }
