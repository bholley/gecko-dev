...
MOZ_ALWAYS_INLINE bool
SetWeakMapEntry(JSContext *cx, Handle<WeakMapObject*> mapObj,
...
WeakMap_set_impl(JSContext *cx, CallArgs args)
{
    ...
    args.rval().setUndefined();
    return SetWeakMapEntry(cx, map, key, value);
}
...
JS_PUBLIC_API(bool)
JS_GetWeakMapEntry(JSContext *cx, HandleObject mapObj, HandleObject key,
...
JS_PUBLIC_API(bool)
JS_SetWeakMapEntry(JSContext *cx, HandleObject mapObj, HandleObject key,
                   HandleValue val)
{
    CHECK_REQUEST(cx);
    assertSameCompartment(cx, key, val);
    Rooted<WeakMapObject*> rootedMap(cx, &mapObj->as<WeakMapObject>());
    return SetWeakMapEntry(cx, rootedMap, key, val);
}
