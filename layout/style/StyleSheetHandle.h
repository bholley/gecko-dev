...
public:
  ...
  // wrapper pointer, rather than putting these methods on StyleSheetHandle
  ...
  StyleSheetHandle(decltype(nullptr) = nullptr) { mPtr.mValue = 0; }
  StyleSheetHandle(const StyleSheetHandle& aOth) { mPtr.mValue = aOth.mPtr.mValue; }
  StyleSheetHandle(CSSStyleSheet* aSet) { *this = aSet; }
  StyleSheetHandle(ServoStyleSheet* aSet) { *this = aSet; }
  StyleSheetHandle(const ::RefPtr<CSSStyleSheet>& aSet) { *this = aSet.get(); }
  StyleSheetHandle(const ::RefPtr<ServoStyleSheet>& aSet) { *this = aSet.get(); }
