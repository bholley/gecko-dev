...
{
  if (nsFrame::GetVerifyStyleTreeEnable() && aPresContext->RestyleManager()->IsGecko()) {
...
PresShell::UpdatePreferenceStyles()
{
  ...
  StyleImplementation styleImplementation = mStyleSet->Implementation();
  RefPtr<StyleSheet> newPrefSheet =
    mPresContext->IsChromeOriginImage() ?
      nsLayoutStylesheetCache::ChromePreferenceSheet(styleImplementation, mPresContext) :
      nsLayoutStylesheetCache::ContentPreferenceSheet(styleImplementation, mPresContext);
...
PresShell::AddUserSheet(nsISupports* aSheet)
{
  ...
  nsTArray<RefPtr<StyleSheet>>& userSheets = *sheetService->UserStyleSheets(mStyleSet->Implementation());
...
PresShell::AddAgentSheet(nsISupports* aSheet)
{
  ...
  // wrt ordering.
  RefPtr<StyleSheet> sheet = do_QueryObject(aSheet);
  if (!sheet) {
    return;
  }
...
PresShell::AddAuthorSheet(nsISupports* aSheet)
{
  ...
  StyleSheet* firstAuthorSheet = mDocument->FirstAdditionalAuthorSheet();
  if (firstAuthorSheet) {
...
PresShell::RecreateFramesFor(nsIContent* aContent)
{
  ...
  nsresult rv = restyleManager->ProcessFrameReconstructions(changeList);
                                  ...
                                  bool aFlushOnHoverChange)
{
  ...
  uint32_t hoverGenerationBefore = restyleManager->AsGecko()->GetHoverGeneration();
                                 ...
                                 EventStates aStateMask)
{
  ...
  if (mStyleSet->IsServo()) {
    // Document state styles are only used in chrome documents.
    return;
  }

  if (mDidInitialize &&
      mStyleSet->AsGecko()->HasDocumentStateDependentStyle(mDocument->GetRootElement(),
                                                           aStateMask)) {
...
FlushThrottledStyles(nsIDocument *aDocument, void *aData)
{
  ...
  if (shell && shell->IsVisible()) {
    ...
    if (presContext && presContext->RestyleManager()->IsGecko()) {
    ...
    if (rootFrame) {
      ...
      if (weakRoot.IsAlive()) {
        ...
        // construction.
        {
          ...
          ++mChangeNestCount;
          Restyler* restyleManager = mPresContext->RestyleManager();
          restyleManager->ProcessFrameReconstructions(changeList);
          if (restyleManager->IsGecko()) {
            restyleManager->AsGecko()->FlushOverflowChangedTracker();
          }
          --mChangeNestCount;
...
PresShell::ListStyleSheets(FILE *out, int32_t aIndent)
{
  ...
  for (int32_t i = 0; i < sheetCount; ++i) {
    StyleSheet* sheet = mStyleSet->StyleSheetAt(SheetType::Doc, i);
    if (sheet->IsGecko()) {
      sheet->AsGecko()->List(out, aIndent);
      fputs("\n", out);
    }
  }
}
                                  ...
                                  size_t *aPresContextSize)
{
  ...
  *aPresShellSize += aArenaObjectsSize->mOther;
  ...
  if (StyleSet()->IsGecko()) {
    *aStyleSetsSize += StyleSet()->AsGecko()->SizeOfIncludingThis(aMallocSizeOf);
  }
                                                      ...
                                                      bool* aRetVal)
{
  ...
  *aRetVal = mStyleSet->IsGecko() ?
    mStyleSet->AsGecko()->HasRuleProcessorUsedByMultipleStyleSets(type) : false;
  return NS_OK;
}
